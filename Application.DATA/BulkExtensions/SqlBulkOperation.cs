...
namespace Portal.Data.BulkExtensions
{
    ...
    internal static class SqlBulkOperation
    {
                ...
                try
                {
                    using (var sqlBulkCopy = GetSqlBulkCopy((SqlConnection)connection, transaction, tableInfo.BulkConfig))
                    {
                        bool useFastMember = tableInfo.HasOwnedTypes == false                      // With OwnedTypes DataTable is used since library FastMember can not (https://github.com/mgravell/fast-member/issues/21)
                                             ...
                                             && tableInfo.ShadowProperties.Count == 0              // With Shadow prop. Discriminator (TPH inheritance) also not used because FastMember is slow for Update (https://github.com/borisdj/Portal.Data.BulkExtensions/pull/17)
        ...
        internal static DataTable GetDataTable<T>(DbContext context, IList<T> entities, SqlBulkCopy sqlBulkCopy, TableInfo tableInfo)
        {
            ...
            string discriminatorColumn = entityType.GetDiscriminatorProperty()?.Name;// tableInfo.ShadowProperties.Count == 0 ? null : tableInfo.ShadowProperties.ElementAt(0);
            ...
            foreach (var property in tableInfo.ShadowProperties)
            {
                if (!properties.Any(h=>h.Name==property))
                {
                    dataTable.Columns.Add(property, typeof(Guid));
                    columnsDict.Add(property, null);
                }
            }
            ...
            foreach (var entity in entities)
            {
                ...
                //{
                //    var property = fk.Properties.Select(h => h.Name).First();
                //    columnsDict[property] = entityType.GetProperties().Where(h=>h.Name==fk.Properties.First().GetColumnName()).Select(h=>h.ValueGenerated); //Todo
                //}
