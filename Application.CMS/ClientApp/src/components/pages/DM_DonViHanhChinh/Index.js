...
import { Select, notification, Input, Skeleton, Card, Col, Row, Layout, Button, Space, Pagination, Form, Modal } from 'antd';
    ...
    async function onMultiDelete() {
        ...
        if (listItemRemove.length == 0) {
            ...
            Modal.confirm({
                ...
                //okButtonProps: { loading: confirmLoading },
                onOk: () => {
                    return postAPI('api/dm_donvihanhchinh/multidelete', formData).then(result => {
                        if (result.status) {
                            setAction(true)
                            notification.success({
                                message: result.message,
                                ...
                                >
                                    <Col xs={{ span: 24 }} lg={{ span: 14 }} md={{ span: 12 }}>
                                        <Form.Item name="Name" label="" style={{ width: '100%' }}>
                                            <Input placeholder="Tên/Mã hành chính" allowClear prefix={<AntdIcons.SearchOutlined />} />
                                        </Form.Item>
                                    </Col>
