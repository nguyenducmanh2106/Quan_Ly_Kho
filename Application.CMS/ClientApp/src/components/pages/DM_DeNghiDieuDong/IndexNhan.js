...
import ExportExcel from "./ExportExcelNhan";
...
function Index({ onSetSanPhamUpdate }) {
  ...
  useEffect(() => {
    async function getData(page, pageSize) {
      ...
      var fetchData = await postAPI(
        `api/dm_denghidieudong/list_data_nhan/`,
        JSON.stringify(obj)
      );
      if (fetchData.status == true) {
  ...
  const onUpdateItem = (item) => {
    history.push({
      ...
      state: {
        controller: "Yêu cầu nhập hàng - Phê duyệt",
        action: "Phê duyệt",
      },
    });
  };
                ...
                >
                  ...
                  <Col xs={{ span: 24 }} lg={{ span: 5 }} md={{ span: 8 }}>
                    ...
                    >
                      ...
                      >
                        <Option value={-1}>-Kho yêu cầu-</Option>
                        {DataDonVi.map((value) => {
                          return (
