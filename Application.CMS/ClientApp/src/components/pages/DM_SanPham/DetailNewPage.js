...
import {
    ...
    Checkbox, Upload, Skeleton, Col, Row, Card, Tooltip, Space, Collapse, Divider, notification, Descriptions, Image,Tabs
} from 'antd';
...
const FormUpdate = () => {
    ...
    useEffect(() => {
        ...
        const getSoLuongSanPham = async () => {
            ...
            var fetchdata = await postAPI(`api/dm_sanpham/thongkesoluong`, JSON.stringify(obj));
            if (fetchdata.status == true) {
                var data = fetchdata.result
                setArray(data)
            }
