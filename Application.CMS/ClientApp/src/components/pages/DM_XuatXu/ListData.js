        ...
        result = array != [] ? array.map((item, index) => {
            return (
                <tr key={item.id} className="ant-table-row ant-table-row-level-0">
                    ...
                    {_isPermission(constantPermission.EDIT, constantPermission.DM_XUATXU) || _isPermission(constantPermission.DELETE, constantPermission.DM_XUATXU) ?
                        <td>
                            <Dropdown placement="bottomCenter" overlay={() => (
                                <Menu>
                                    {_isPermission(constantPermission.EDIT, constantPermission.DM_XUATXU) ?
