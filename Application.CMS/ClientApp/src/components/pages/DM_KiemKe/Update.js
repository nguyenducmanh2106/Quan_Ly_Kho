...
const ModalCreate = () => {
    ...
    const [kiemKe, setKiemKe] = useState({});
    ...
    useEffect(() => {
        ...
        async function getData() {
            ...
            if (fetchData.status == true) {
                ...
                console.log(data)
                setKiemKe(data)
                setDataSanPhamSubmit(data.chiTietKiemKes)
                form.setFieldsValue({
                    GhiChu: data.ghiChu,
                    ID_ChiNhanh: data.id_ChiNhanh,
                    NhanVienKiem: data.nhanVienKiem
                })
            }
        }
        //gọi hàm
    ...
    async function onPostCreateItem(obj) {
        console.log(obj)
        setIsLoading(true)
        var result = await postAPI('api/dm_kiemke/update', JSON.stringify(obj))
        if (result.status) {
            ...
            if (fetchData.status == true) {
                ...
                if (DataSanPhamSubmit.length > 0) {
                    DataSanPhamSubmit.map(item => {
                        item.ID_SanPham = item.iD_SanPham
    ...
    const renderBody = () => {
        ...
        result = DataSanPhamSubmit.map((item, index) => {
            item.soLuongTrongKho = item.tonChiNhanh ?? item.soLuongTrongKho
            item.name = item.tenSanPham ?? item.name
            return (
                        ...
                        <Card
                            ...
                            <Row gutter={10}>
                                <Col xs={{ span: 24 }} md={{ span: 24 }} lg={{ span: 8 }}>
                                    <Descriptions >
                                        <Descriptions.Item label="Tên chi nhánh">{kiemKe.tenChiNhanh}</Descriptions.Item>
                                    </Descriptions>
                                </Col>
                                <Col xs={{ span: 24 }} md={{ span: 24 }} lg={{ span: 8 }}>
                                    <Descriptions >
                                        <Descriptions.Item label="Nhân viên tạo">{kiemKe.tenNguoiTao}</Descriptions.Item>
                                    </Descriptions>
                                    <Descriptions >
                                        <Descriptions.Item label="Nhân viên kiểm">{kiemKe.tenNguoiKiem}</Descriptions.Item>
                                    </Descriptions>
                                </Col>
                                <Col xs={{ span: 24 }} md={{ span: 24 }} lg={{ span: 8 }}>
