...
function Index({ onSetSanPhamUpdate }) {
  ...
  async function onHandleSearch(data) {
    ...
    var NgayKiem = data.NgayKiem
      ? moment(data.NgayKiem).format("YYYY/MM/DD")
      : "";
  ...
  const onDelete = (item) => {
    Modal.confirm({
      ...
      onOk: () => {
        return postAPI("api/dm_kiemke/delete", JSON.stringify(item)).then(
          (result) => {
            if (result.status) {
              setAction(true);
  ...
  async function onMultiDelete() {
    if (listItemRemove.length == 0) {
      ...
      Modal.confirm({
        ...
        //okButtonProps: { loading: confirmLoading },
        onOk: () => {
          return postAPI("api/dm_kiemke/multidelete", formData).then(
            (result) => {
              if (result.status) {
                setAction(true);
                notification.success({
          ...
          <Skeleton loading={isLoading} active paragraph={false}>
            ...
            >
              ...
              >
                ...
                <Row gutter={16}>
                  <Col lg={{ span: 12 }} md={{ span: 24 }} xs={{ span: 24 }}>
                    <Form.Item name="NgayKiem" label="Ngày kiểm hàng">
                      <DatePicker
                        style={{ width: "100%" }}
                        getPopupContainer={(trigger) => trigger.parentElement}
                  ...
                  <Col lg={{ span: 12 }} md={{ span: 24 }} xs={{ span: 24 }}>
                    <Form.Item name="TypeFilterNgayKiem" label="Kiểu lọc ngày">
                      <Select placeholder="Please choose the type">
                        <Option value={-1}>-- Chon --</Option>
                        <Option value={0}>Bằng</Option>
                        <Option value={1}>Lớn hơn hoặc bằng</Option>
