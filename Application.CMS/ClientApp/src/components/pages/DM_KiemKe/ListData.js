...
function Table(props) {
    ...
    const onShow = (item) => {
        history.push({
            pathname: `/dm_kiemke/view/${item.code}`,
            state: { controller: "Kiểm hàng", action: item.code }
        });
    }
    const update = (item) => {
                    ...
                    <tr key={item.id} className="ant-table-row ant-table-row-level-0">
                        ...
                        <td style={{ textAlign: "center" }}>
                            {item.status == 1 ?
                                ...
                                : item.status == 2 ? <Typography.Text type="success">Đang cân bằng kho</Typography.Text> : item.status == 0 ?
                                    <Typography.Text type="warning">Tạo đơn</Typography.Text> : item.status == 3 ?
                                        <Typography.Text type="success">Hoàn thành</Typography.Text> : item.status == 4 ?
                                            <Typography.Text type="danger">Đang kiểm kho</Typography.Text> : ""
                            }
                        </td>
                        <td>
                        ...
                        <td>
                            {item.ngayKiem ? moment(item.ngayKiem).format('DD/MM/YYYY, HH:mm') : ""}
                        </td>
                        <td>
                            {item.tenNguoiTao}
                        </td>
