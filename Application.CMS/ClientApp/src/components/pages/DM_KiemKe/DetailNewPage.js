...
const ModalCreate = () => {
    ...
    for (var i = 0; i < sp.length; i++) {
      var object = {
        ...
        TonThucTe: sp[i]
          ...
          .querySelector(".ant-input-number-input").value
          ? Number.parseInt(
              sp[i]
                .querySelector(".SoLuong")
                .querySelector(".ant-input-number-input").value
      ...
      result = DataSanPham.map((item, index) => {
        ...
        return (
          <tr key={item.id} className="ant-table-row ant-table-row-level-0">
            ...
            <td id={item.ID_SanPham}>{item.tonChiNhanh}</td>
      ...
      result = DataSanPham.map((item, index) => {
        ...
        return (
          <tr key={item.id} className="ant-table-row ant-table-row-level-0">
            ...
            <td>{item.tenDonViTinh}</td>
            <td id={item.ID_SanPham}>{item.tonChiNhanh}</td>
            <td className="SoLuong">{item.tonThucTe}</td>
            <td className="GhiChu">{item.ghiChu}</td>
          </tr>
        );
      });
    }
      ...
      result = DataSanPham.map((item, index) => {
        ...
        return (
          <tr key={item.id} className="ant-table-row ant-table-row-level-0">
            ...
            <td>{item.tenDonViTinh}</td>
            <td>{item.tonChiNhanh}</td>
            <td className="SoLuong">{item.tonThucTe}</td>
            <td>{item.tonThucTe - item.tonChiNhanh}</td>
            <td>{item.ghiChu}</td>
          </tr>
        );
      });
  ...
  const menu = (
    <Menu>
      {_isPermission(PERMISSION.EDIT, PERMISSION.DM_KIEMKE) &&
      ...
      <ReactToPrint
          trigger={() => (
            <a href="javascript:void(0)">
              <AntdIcons.PrinterOutlined /> In đơn kiểm hàng
            </a>
          )}
          content={() => componentRef.current}
          documentTitle={"PhieuKiemHang_"+history.location.state.action}
      ...
      >
          ...
          <Col xs={{ span: 24 }} md={{ span: 24 }} lg={{ span: 18 }}>
            ...
            >
              ...
              <Steps.Step
                title="Kiểm hàng"
                description={
                  nhapHang.ngayKiem && nhapHang?.status != 4
                    ? moment(nhapHang.ngayKiem).format("DD/MM/YYYY, HH:mm")
                    : ""
                }
              />
              <Steps.Step
                title="Thống kê"
                description={
                  nhapHang.ngayCanBang && nhapHang?.status != 4
                    ? moment(nhapHang.ngayCanBang).format("DD/MM/YYYY, HH:mm")
                    : ""
                }
              />
              <Steps.Step
                ...
                description={
                  nhapHang.ngayHoanThanh
                    ? moment(nhapHang.ngayHoanThanh).format("DD/MM/YYYY, HH:mm")
                    : ""
                }
              />
            </Steps>
                    ...
                    <Col>
                      <Descriptions>
                        <Descriptions.Item label="Nhân viên tạo">
                          {nhapHang.tenNguoiTao}
                        </Descriptions.Item>
                      </Descriptions>
                    </Col>
                    <Col>
                      <Descriptions>
                        <Descriptions.Item label="Nhân viên kiểm">
                          {nhapHang.tenNguoiKiem}
                        </Descriptions.Item>
                      </Descriptions>
                    </Col>
                  </Col>
                    ...
                    <Col>
                      <Descriptions>
                        <Descriptions.Item label="Ngày tạo">
                          {nhapHang.created_At
                            ? moment(nhapHang.created_At).format(
                                "DD/MM/YYYY, HH:mm"
                              )
                            : ""}
                        </Descriptions.Item>
                    ...
                    <Col>
                      <Descriptions>
                        <Descriptions.Item label="Ngày kiểm">
                          {nhapHang.ngayKiem
                            ? moment(nhapHang.ngayKiem).format(
                                "DD/MM/YYYY, HH:mm"
                              )
                            : ""}
                        </Descriptions.Item>
                      ...
                      {nhapHang.status == 2 ? (
                        <Descriptions>
                          <Descriptions.Item label="Ngày cân bằng">
                            {nhapHang.ngayCanBang
                              ? moment(nhapHang.ngayCanBang).format(
                                  "DD/MM/YYYY, HH:mm"
                                )
                              : ""}
                          </Descriptions.Item>
                            ...
                            >
                              <thead className="ant-table-thead">
                                <tr>
                                  ...
                                  <th className="">Tồn chi nhánh</th>
                                  {nhapHang.status == 1 ||
                                  nhapHang.status == 4 ? (
                                    <>
                                      <th className="">Tồn thực tế</th>
        ...
        <Row>
          <Col style={{ textAlign: "right" }}>
            {nhapHang?.status == 0 &&
            _isPermission(PERMISSION.DUYET, PERMISSION.DM_KIEMKE) ? (
              <Button
                type="primary"
                icon={<AntdIcons.SaveOutlined />}
                onClick={() => DangKiemHang(nhapHang)}
              >
                Kiểm hàng
              </Button>
            ) : nhapHang?.status == 4 &&
