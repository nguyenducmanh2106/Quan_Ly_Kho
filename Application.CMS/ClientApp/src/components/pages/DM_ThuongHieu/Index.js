...
function Menu() {
    ...
    async function onHandleSearch(data) {
        ...
        var fetchData = await getAPI(`api/dm_thuonghieu/list_data?Name=${name}&Status=${status}&page=${page}&pageSize=${pageSize}&nameSort=${nameSort}`);
        if (fetchData.status == true) {
            setState(fetchData.result)
        }
    }
    ...
    async function onMultiDelete() {
        if (listItemRemove.length == 0) {
            ...
            Modal.confirm({
                ...
                //okButtonProps: { loading: confirmLoading },
                onOk: () => {
                    return postAPI('api/dm_thuonghieu/multidelete', formData).then(result => {
                        if (result.status) {
                            setAction(true)
                            notification.success({
                                message: result.message,
            ...
            <Card>
                <Row>
                                ...
                                >
                                    <Col xs={{ span: 24 }} lg={{ span: 14 }} md={{ span: 12 }}>
                                        <Form.Item name="Name" label="" style={{ width: '100%' }}>
                                            <Input placeholder="Tên/Mã thương hiệu" allowClear prefix={<AntdIcons.SearchOutlined />} />
                                        </Form.Item>
                                    </Col>
                    ...
                    <Col xs={{ span: 24 }} lg={{ span: 24 }} style={{ marginBottom: "16px" }}>
                        <Skeleton loading={isLoading} active>
                            <Space size={8}>
                                {_isPermission(constantPermission.CREATE, constantPermission.DM_THUONGHIEU) ?
                                    <Button type="primary" className="success" onClick={toggle} icon={<AntdIcons.PlusOutlined />}>
                                        Thêm mới
                                    </Button> : null
                                }
                                {_isPermission(constantPermission.DELETE, constantPermission.DM_THUONGHIEU) ?
