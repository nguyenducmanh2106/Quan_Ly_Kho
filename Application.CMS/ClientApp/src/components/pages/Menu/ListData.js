        ...
        result = array != [] ? array.map((item, index) => {
            return (
                <tr key={item.id} className="ant-table-row ant-table-row-level-0">
                    ...
                    {_isPermission(constantPermission.EDIT, constantPermission.MENU) || _isPermission(constantPermission.DELETE, constantPermission.MENU) ?
                        <td>
                            <Dropdown placement="bottomCenter" overlay={() => (
                                <Menu>
                                    {_isPermission(constantPermission.EDIT, constantPermission.MENU) ?
