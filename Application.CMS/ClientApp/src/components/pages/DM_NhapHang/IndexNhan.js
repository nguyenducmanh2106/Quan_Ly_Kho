...
function Index({ onSetSanPhamUpdate }) {
  ...
  const onDelete = (item) => {
    Modal.confirm({
      ...
      //okButtonProps: { loading: confirmLoading },
      onOk: () => {
        return postAPI("api/dm_nhaphang/delete", JSON.stringify(item)).then(
          (result) => {
            if (result.status) {
              setAction(true);
              notification.success({
  ...
  async function onMultiDelete() {
    if (listItemRemove.length == 0) {
      ...
      Modal.confirm({
        ...
        //okButtonProps: { loading: confirmLoading },
        onOk: () => {
          return postAPI("api/dm_nhaphang/multidelete", formData).then(
            (result) => {
              if (result.status) {
                setAction(true);
                notification.success({
            ...
            <Skeleton loading={isLoading} active>
              <Space size={8}>
                {_isPermission(
                  ...
                  constantPermission.DM_NHAPHANG
                ) ? (
                  <Button
                    type="primary"
                    className="success"
