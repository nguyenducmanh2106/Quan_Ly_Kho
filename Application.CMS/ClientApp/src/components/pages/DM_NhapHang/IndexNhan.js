...
function Index({ onSetSanPhamUpdate }) {
    ...
    async function onUpdateItemPosition(ItemPosition) {
        ...
        if (ItemPosition.ordering < 0 || Number.isNaN(ItemPosition.ordering)) {
            ...
            var result = await postAPI('api/dm_nhaphang/update', JSON.stringify(ItemPosition))
            if (result.status) {
                setAction(true)
    ...
    const onDelete = (item) => {
        Modal.confirm({
            ...
            //okButtonProps: { loading: confirmLoading },
            onOk: () => {
                return postAPI('api/dm_nhaphang/delete', JSON.stringify(item)).then(result => {
                    if (result.status) {
                        setAction(true)
                        notification.success({
                            message: result.message,
    ...
    async function onMultiDelete() {
        if (listItemRemove.length == 0) {
            ...
            Modal.confirm({
                ...
                //okButtonProps: { loading: confirmLoading },
                onOk: () => {
                    return postAPI('api/dm_nhaphang/multidelete', formData).then(result => {
                        if (result.status) {
                            setAction(true)
                            notification.success({
                                message: result.message,
            ...
            <Card>
                <Row>
                                    ...
                                    <Col xs={{ span: 24 }} lg={{ span: 6 }} md={{ span: 8 }}>
                                        <Form.Item name="Status" label="" style={{ width: '100%' }}>
                                            ...
                                            >
                                                ...
                                                <Option value={2}>Đang nhập kho</Option>
                    ...
                    <Col xs={{ span: 24 }} lg={{ span: 24 }} style={{ marginBottom: "16px" }}>
                        <Skeleton loading={isLoading} active>
                            <Space size={8}>
                                {_isPermission(constantPermission.CREATE, constantPermission.DM_NHAPHANG) ?
                                    <Button type="primary" className="success" onClick={openCreate} icon={<AntdIcons.PlusOutlined />}>
                                        Thêm mới
                                    </Button> : null
                                }
                                {_isPermission(constantPermission.DELETE, constantPermission.DM_NHAPHANG) ?
