...
function Index() {
    ...
    useEffect(() => {
        async function getData(page, pageSize) {
            ...
            var fetchData = await getAPI(`api/dm_nhomthuoctinh/list_data/?Name=${name}&Status=${status}&page=${page}&pageSize=${pageSize}&nameSort=${nameSort}`);
    ...
    async function onUpdateItemPosition(ItemPosition) {
        ...
        if (ItemPosition.ordering < 0 || Number.isNaN(ItemPosition.ordering)) {
            ...
            var result = await postAPI('api/dm_nhomthuoctinh/update', JSON.stringify(ItemPosition))
            if (result.status) {
                setAction(true)
    ...
    const onToggleStatus = (item) => {
        Modal.confirm({
            ...
            onOk: () => {
                return postAPI('api/dm_nhomthuoctinh/update', JSON.stringify(item)).then(result => {
                    if (result.status) {
                        setAction(true)
                        setConfirmLoading(false)
                        notification.success({
    ...
    const onDelete = (item) => {
        Modal.confirm({
            ...
            //okButtonProps: { loading: confirmLoading },
            onOk: () => {
                return postAPI('api/dm_nhomthuoctinh/delete', JSON.stringify(item)).then(result => {
                    if (result.status) {
                        setAction(true)
                        notification.success({
                            message: result.message,
    ...
    async function onMultiDelete() {
        ...
        if (listItemRemove.length == 0) {
            ...
            Modal.confirm({
                ...
                //okButtonProps: { loading: confirmLoading },
                onOk: () => {
                    return postAPI('api/dm_nhomthuoctinh/multidelete', formData).then(result => {
                        if (result.status) {
    ...
    async function onPostCreateItem(obj) {
        setConfirmLoading(true)
        var result = await postAPI('api/dm_nhomthuoctinh/create', JSON.stringify(obj))
        if (result.status) {
            setAction(true)
            toggle();
            notification.success({
            ...
            <Card>
                <Row>
                                ...
                                >
                                    <Col xs={{ span: 24 }} lg={{ span: 14 }} md={{ span: 12 }}>
                                        <Form.Item name="Name" label="" style={{ width: '100%' }}>
                                            <Input placeholder="Tên/Mã nhóm thuộc tính" allowClear prefix={<AntdIcons.SearchOutlined />} />
                                        </Form.Item>
                                    </Col>
                    ...
                    <Col xs={{ span: 24 }} lg={{ span: 24 }} style={{ marginBottom: "16px" }}>
                        <Skeleton loading={isLoading} active>
                            <Space size={8}>
                                {_isPermission(constantPermission.CREATE, constantPermission.DM_NHOMTHUOCTINH) ?
                                    <Button type="primary" className="success" onClick={toggle} icon={<AntdIcons.PlusOutlined />}>
                                        Thêm mới
                                    </Button> : null
                                }
                                {_isPermission(constantPermission.DELETE, constantPermission.DM_NHOMTHUOCTINH) ?
